...
Respond again with ONLY the corrected JSON object, no markdown, no commentary.`;
  ...
  // Includes a full rewritten exampleOptimizedSolution in the output, so
  // this needs the same larger token budget as extractKnowledge.
  async reviewCode(input: ReviewCodeInput): Promise<CodeReviewResult> {
    const { system, user } = buildCodeReviewPrompt(input);
    return this.callAndValidate(system, user, CodeReviewSchema, 8192);
  }
}
